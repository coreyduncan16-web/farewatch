...
function flexPanel(o, d, pair){
  ...
  h += '<table><thead><tr><th>Date</th><th>' + (basis === 'gw' ? 'GoWild' : 'Cash') + '</th><th>vs cheapest</th><th>Vs. local range</th><th></th></tr></thead><tbody>';
  days.forEach(function(r){
    ...
    var overHtml = '<span class="delta">&mdash;</span>';
    if (r._v !== null && lo > 0) {
      var over = Math.round((r._v - lo) / lo * 100);
      overHtml = over <= 0 ? '<span class="sig BUY">bottom $</span>' : '<span class="delta up">+' + over + '%</span>';
    }
    h += '<tr class="row"><td class="num">' + r.dep + '</td>'
      + '<td class="num">' + (r._v !== null ? money(r._v) : '&mdash;') + '</td>'
      + '<td class="num">' + overHtml + '</td>'
      + '<td>' + verdict + '</td>'
